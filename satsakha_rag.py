...
"""
SatSakha RAG Demo
- Connects to Neo4j (AuraDB or local Neo4j)
- Retrieves region-specific facts (NDVI, rainfall, advisories)
- Sends context + user question to Hugging Face Inference API
...
HF_MODEL = "distilgpt2"  # good trade-off between speed & accuracy
...
#
# # ------------------- Hugging Face API -------------------
# HF_TOKEN = ""   # 🔴 Hardcode temporarily to debug
# HF_MODEL = "mistralai/Mistral-7B-Instruct-v0.3"                      # hosted free model
...
# """
#
# def query_hf(prompt, retries=5, wait=5):
#     """Send request to Hugging Face API with retries."""
...
#         else:
#             print(f"[Retry {i+1}] HF API error {response.status_code}: {response.text}")
#             time.sleep(wait)
#     raise RuntimeError("Hugging Face API failed after retries")
#
# def answer_question(question, region_code="IN-MH-WRD", period_key=None):
#     facts = build_context(region_code, period_key)
#     prompt = PROMPT_TEMPLATE.format(facts=facts, question=question)
#
#     hf_output = query_hf(prompt)
#     # HF returns a list of dicts, extract generated_text
#     if isinstance(hf_output, list) and "generated_text" in hf_output[0]:
#         return hf_output[0]["generated_text"]
#     else:
#         return str(hf_output)
#
...
HF_MODEL = "gpt2"   # OR "distilgpt2" if you prefer GPT-style
...
#     print("SatSakha Demo (Neo4j + Hugging Face API)")
#     print("Ask about NDVI, rainfall, or advisories for Wardha (code IN-MH-WRD).")
#     while True:
...
if __name__ == "__main__":
    ...
    test_q = "Which month has best NDVI score in Wardha"
    print(f"\n[Test Input] {test_q}")
